...
// 対象動画（環境変数で上書き可能）
const VIDEO_ID = process.env.VIDEO_ID || "eKC3_9gRrOI";
// ホーム→動画 のSPA遷移用の検索クエリ。動画IDで検索すると当該動画が結果に出る。
const SEARCH_QUERY = process.env.SEARCH_QUERY || VIDEO_ID;

const HOME_URL = "https://www.youtube.com/?hl=en";
const WATCH_URL = `https://www.youtube.com/watch?v=${VIDEO_ID}&hl=en`;
...
test("ケースA: ホーム→検索でSPA遷移→動画→コメントのタイムスタンプ", async ({ context }) => {
    ...
    const result = page.locator(`a#video-title[href*="watch?v=${VIDEO_ID}"]`).first();
    await expect(result).toBeVisible({ timeout: 30_000 });
    await result.click();

    await expect(page).toHaveURL(new RegExp(`watch\\?v=${VIDEO_ID}`), { timeout: 30_000 });
